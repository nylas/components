...
Nylas Schedule Editor (`<nylas-schedule-editor>`) lets you build event-booking functionality into your application in minutes. Use Nylas Schedule Editor with your Nylas account or by passing in your own JSON data.
...
First, install the Nylas Schedule Editor Component with npm

```bash
npm install @nylas/components-schedule-editor
```

or with yarn

```bash
yarn add @nylas/components-schedule-editor
```
...
If you've installed Nylas Schedule Editor using the CDN, then `<nylas-schedule-editor></nylas-schedule-editor>` is ready to use.

Otherwise, import the Nylas Schedule Editor component into your application.

```js
import "@nylas/components-schedule-editor";
```
...
Nylas Schedule Editor can be used as a UI for any data you provide.

Properties you can pass to Nylas Schedule Editor are [`event_title`](<(#properties)>), [`event_description`](#properties), [`event_location`](#properties), [`event_conferencing`](#properties), [`slots_to_book`](#properties)
...
| ---------------------- | ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------- |
| id                     | string              | The id of the Editor component                                                                                                                                                                                                                             | true     |                                                                   |
...
| allow_booking          | boolean             | Controls if time slots are bookable when used with the Nylas Booking and Availability components.                                                                                                                                                          | false    | false                                                             |
...
| show_preview           | boolean             | Toggles a preview of the editor settings in Availability and Booking components.                                                                                                                                                                           | false    |                                                                   |
...
| events                 | EventDefinition[]   | A list of event(s) to be booked. If multiple events are provided, the component will attempt to book them consecutively.                                                                                                                                   | false    | []                                                                |
| custom_fields          | CustomField[]       | Add custom fields to collect input data from users before booking                                                                                                                                                                                          | false    | By default, the Booking component adds inputs for name, and email |
...
```js
document
  .querySelector("nylas-schedule-editor")
...
### Using Schedule Editor Configuration with Nylas Booking

You can configure the Nylas Booking Component to use the settings from the Schedule Editor by setting the Schedule Editor ID.
