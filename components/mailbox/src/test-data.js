var thread1 = {
  account_id: "1xrddnl99frq3b7son9j32aba",
  drafts: [],
  first_message_timestamp: 1629832405,
  has_attachments: false,
  id: "dlrwnv5oc3v3apxc2j0ht8vop",
  labels: [
    { display_name: "Inbox", id: "dx62wkpj57erbkargbr3zew3j", name: "inbox" },
  ],
  last_message_received_timestamp: 1629832405,
  last_message_sent_timestamp: null,
  last_message_timestamp: 1629832405,
  messages: [
    {
      account_id: "1xrddnl99frq3b7son9j32aba",
      bcc: [],
      cc: [],
      date: 1629832405,
      files: [],
      from: [{ email: "tips@nylas.com", name: "J Valdivia" }],
      id: "3ozrqu3obfwzmsltxj72ll86m",
      labels: [
        {
          display_name: "Inbox",
          id: "dx62wkpj57erbkargbr3zew3j",
          name: "inbox",
        },
      ],
      object: "message",
      reply_to: [{ email: "tips@nylas.com", name: "" }],
      snippet:
        "Set yourself up for an integration that scales by avoiding common roadblocks and pitfalls. Learn how to boost revenue, accelerate your roadmap, and meet & exceed your customers’ needs with these assets.",
      starred: false,
      subject: "API Integration Pitfalls and How to Avoid Them",
      thread_id: "dlrwnv5oc3v3apxc2j0ht8vop",
      to: [{ email: "nylascypresstest+trialuser2@gmail.com", name: "" }],
      unread: true,
    },
  ],
  object: "thread",
  participants: [
    { email: "tips@nylas.com", name: "J Valdivia" },
    { email: "nylascypresstest+trialuser2@gmail.com", name: "" },
  ],
  snippet:
    "Set yourself up for an integration that scales by avoiding common roadblocks and pitfalls. Learn how to boost revenue, accelerate your roadmap, and meet & exceed your customers’ needs with these assets.",
  starred: false,
  subject: "API Integration Pitfalls and How to Avoid Them",
  unread: true,
  version: 2,
};
var thread2 = {
  account_id: "1xrddnl99frq3b7son9j32aba",
  drafts: [],
  first_message_timestamp: 1628180118,
  has_attachments: false,
  id: "f4k41gllbmv47xrmjt4me2tcu",
  labels: [
    { display_name: "Inbox", id: "dx62wkpj57erbkargbr3zew3j", name: "inbox" },
  ],
  last_message_received_timestamp: 1628180118,
  last_message_sent_timestamp: null,
  last_message_timestamp: 1628180118,
  messages: [
    {
      account_id: "1xrddnl99frq3b7son9j32aba",
      bcc: [],
      cc: [],
      date: 1628180118,
      files: [],
      from: [{ email: "tips@nylas.com", name: "Matt Kearns" }],
      id: "5ntry008gmhg1c8ra4gg36y9h",
      labels: [
        {
          display_name: "Inbox",
          id: "dx62wkpj57erbkargbr3zew3j",
          name: "inbox",
        },
      ],
      object: "message",
      reply_to: [{ email: "tips@nylas.com", name: "" }],
      snippet:
        "The more you refer, the more you save! Now you can earn credits for referring your network to Nylas! The larger the deal size of your referee, the larger your Nylas credits: $200 Nylas Credit",
      starred: false,
      subject: "Credits for Referring Nylas!",
      thread_id: "f4k41gllbmv47xrmjt4me2tcu",
      to: [
        {
          email: "nylascypresstest+arjunandpoojatestthethankyoupage@gmail.com",
          name: "",
        },
      ],
      unread: false,
    },
  ],
  object: "thread",
  participants: [
    { email: "tips@nylas.com", name: "Matt Kearns" },
    {
      email: "nylascypresstest+arjunandpoojatestthethankyoupage@gmail.com",
      name: "",
    },
  ],
  snippet:
    "The more you refer, the more you save! Now you can earn credits for referring your network to Nylas! The larger the deal size of your referee, the larger your Nylas credits: $200 Nylas Credit",
  starred: false,
  subject: "Credits for Referring Nylas!",
  unread: false,
  version: 68,
};

export { thread1, thread2 };
