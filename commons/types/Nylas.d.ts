declare namespace Nylas {
  interface CommonQuery {
    component_id: string;
    access_token?: string;
  }

  interface ConversationQuery extends CommonQuery {
    thread_id: string;
  }

  interface MailboxQuery extends CommonQuery {
    query: ThreadsQuery;
  }
  ...
  interface MessagesQuery extends CommonQuery {
    received_before?: number;
    received_after?: number;
  }

  interface CleanConversationQuery {
    component_id: string;
    message_id: string[]; // Note: singular name but expects array
  }

  interface CleanConversationFeedbackQuery {
    component_id: string;
    message_id: string;
  }
  ...
  interface Participant {
    ...
    contact?: Contacts.HydratedContact | null;
  }
  ...
  interface RadialMessage extends Message {
  ...
  interface NError extends Error {
    name: import("../src/components/NErrorMapping").NErrorType[number];
  }
  ...
  interface AgendaProperties extends Manifest {
  ...
  interface EmailProperties extends Manifest {
  ...
  interface ComposerProperties extends Manifest {
    show_header?: boolean;
    theme?: "light" | "dark";
    mode?: "inline" | "popup";
    show_from?: boolean;
  ...
  interface ContactListProperties extends Manifest {
  ...
  interface ConversationProperties extends Manifest {
    theme: "theme-1" | "theme-2" | "theme-3" | "theme-4";
    show_avatars: boolean | string;
    show_reply: boolean | string;
  }

  interface DayProperties extends Manifest {
  ...
  interface Conversation extends Thread {
    messages: Message[];
  }
